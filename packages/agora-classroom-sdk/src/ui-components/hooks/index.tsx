...
import {
  calcUploadFilesMd5,
  uploadFileInfoProps,
} from '../common-containers/cloud-driver';
...
export const useChatContext = () => {
  ...
  const refreshMessageList = useCallback(async () => {
    const res =
      nextId !== 'last' &&
      (await roomStore.getHistoryChatMessage({ nextId, sort: 0 }));
  ...
  const onCanChattingChange = useCallback(
    async (canChatting: boolean) => {
      if (canChatting) {
...
export const useSettingContext = () => {
  ...
  const onSelectMirror = useCallback(
    (evt: any) => {
      setMirror(!isMirror);
    },
...
export const useCloudDriverContext = (props: any) => {
  ...
  const handleUpload = async (evt: any) => {
    ...
    const payload = {
      ...
      pptConverter: boardStore.boardClient.client.pptConverter(
        boardStore.room.roomToken,
      ),
    };
    if (ext === 'pptx') {
...
export const useUploadContext = (
  handleUpdateCheckedItems: CallableFunction,
) => {
  const boardStore = useBoardStore();
...
export const useHandsUpManager = () => {
  ...
  const handleUpdateList = useCallback(
    async (type: string, info: StudentInfo) => {
      if (type === 'confirm') {
...
export const useWhiteboardState = () => {
  ...
  return {
    ...
    showZoomControl,
    showTab:
      roomStore.roomInfo.userRole === EduRoleTypeEnum.student ? false : true,
  };
};
...
export const useDownloadContext = () => {
  ...
  const itemList = boardStore.downloadList.filter(
