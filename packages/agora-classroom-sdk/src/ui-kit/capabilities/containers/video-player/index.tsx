import { ControlTool, EduMediaStream, useGlobalContext, useRoomContext, useSmallClassVideoControlContext, usePrivateChatContext, useStreamListContext, useUserListContext, useVideoControlContext, usePretestContext, useVolumeContext } from 'agora-edu-core';
...
export const VideoPlayerTeacher = observer(({ style, className, controlPlacement = 'left', placement = 'left' }: any) => {
  ...
  return (
    <VideoPlayer
      ...
      className={className}
      renderVolumeIndicator={() => <CustomizeVolumeIndicator streamUuid={userStream.streamUuid} />}
      style={style}>
      {
        <>
          {userStream.renderer && userStream.video ? <RendererPlayer key={userStream.renderer && userStream.renderer.videoTrack ? userStream.renderer.videoTrack.getTrackId() : ''} track={userStream.renderer} id={userStream.streamUuid} className="rtc-video" /> : null}
...
export const VideoMarqueeStudentContainer = observer(() => {
  ...
  const videoStreamList = useMemo(() => {
    return studentStreams.map((stream: EduMediaStream) => ({
      ...
      children: (
        <>
          {stream.renderer && stream.video ? <RendererPlayer key={stream.renderer && stream.renderer.videoTrack ? stream.renderer.videoTrack.getTrackId() : ''} track={stream.renderer} id={stream.streamUuid} className="rtc-video" /> : null}
  ...
  return videoStreamList.length ? (
    <div className="video-marquee-pin">
      <VideoMarqueeList teacherStreams={[]} hideStars={sceneType === 2} videoStreamList={videoStreamList} userType={eduRole2UIRole(roomInfo.userRole)} onCameraClick={onCameraClick} onMicClick={onMicClick} onSendStar={onSendStar} onWhiteboardClick={onWhiteboardClick} onOffPodiumClick={onOffPodiumClick} onPrivateChat={onPrivateChat} />
    </div>
  ) : null;
});
...
export const MidVideoMarqueeContainer = observer(() => {
  ...
  return videoStreamList.length || teacherVideoList.length ? (
    <div className="video-marquee-pin">
      <VideoMarqueeList openCarousel={!!roomProperties.carousel?.state} teacherStreams={teacherVideoList.length ? teacherVideoList : []} hideStars={sceneType === 2} videoStreamList={videoStreamList.length ? videoStreamList : []} userType={eduRole2UIRole(roomInfo.userRole)} onCameraClick={onCameraClick} onMicClick={onMicClick} onSendStar={onSendStar} onWhiteboardClick={onWhiteboardClick} onOffPodiumClick={onOffPodiumClick} onPrivateChat={onPrivateChat} />
    </div>
  ) : null;
});
