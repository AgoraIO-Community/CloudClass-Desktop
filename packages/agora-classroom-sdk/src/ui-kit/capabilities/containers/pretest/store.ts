...
export class PretestUIStore extends PretestUIKitStore {
  ...
  static createFactory(sceneStore: SceneStore, payload?: PretestModel) {
    const store = new PretestUIStore(sceneStore)
  ...
  @computed
  get microphoneLevel() {
    return this.sceneStore.mediaStore.totalVolume
  }
  ...
  @computed
  get speakerList(): any[] {
    // if (this.sceneStore.uiStore.isElectron) {
  ...
  get mediaService(): MediaService {
    return this.sceneStore.eduManager.mediaService as MediaService;
  }
  ...
  async changeWebCamera(deviceId: string) {
    if (this.sceneStore.cameraRenderer) {
      ...
      this.sceneStore.mediaStore.updateCameraLabel(label)
    } else {
      ...
      this.sceneStore.mediaStore._cameraRenderer = this.mediaService.cameraRenderer
      const label = this.mediaService.getCameraLabel()
  ...
  async changeWebMicrophone(deviceId: string) {
    if (this.sceneStore.mediaStore._microphoneTrack) {
      ...
      if (!cameraEduStream || !!cameraEduStream.hasAudio === false) {
        ...
        const deviceLabel = this.sceneStore.mediaStore.getAudioDeviceLabelBy(deviceId)
        if (deviceLabel) {
      ...
      this.sceneStore.mediaStore._microphoneTrack = this.mediaService.microphoneTrack
      const label = this.mediaService.getMicrophoneLabel()
      this.updateMicrophoneLabel(label)
    }
  }
  ...
  async changeCamera(deviceId: string) {
    if (deviceId === AgoraMediaDeviceEnum.Default) {
      ...
      let sceneCameraRenderer = this.sceneStore.mediaStore._cameraRenderer
