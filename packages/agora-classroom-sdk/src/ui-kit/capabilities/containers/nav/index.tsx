...
const SignalQuality = visibilityControl(observer(() => {
  const { navigationBarUIStore } = useStore();
  ...
  return (
    <Popover content={<SignalContent />} placement="bottomLeft">
      <div className={`biz-signal-quality ${networkQualityClass}`}>
        <SvgImg className="cursor-pointer" type={networkQualityIcon.icon} colors={{ iconPrimary: networkQualityIcon.color }} size={24} />
      </div>
    </Popover>
  );
}), networkStateEnabled);
...
const ScheduleTime = visibilityControl(observer(() => {
  const { navigationBarUIStore } = useStore();
...
const RoomName = visibilityControl(observer(() => {
  const { navigationBarUIStore } = useStore();
...
const StartButton = observer(() => {
  const { navigationBarUIStore } = useStore();
  const { isBeforeClass, startClass } =
...
export const NavigationBarAction = visibilityListItemControl(observer(({ action }: { action: EduNavAction }) => {
  return (
