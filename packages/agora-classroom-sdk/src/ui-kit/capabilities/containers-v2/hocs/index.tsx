...
const DeviceDropdown = observer(
  ({ disabled, muted, icon, onDeviceClick, dropdownContent }: DeviceDropdownProps) => {
        ...
        {!disabled && (
          <a
            ...
            style={{
              ...
              borderRadius: 6,
              padding: '0 5px',
              background: arrowButtonBackground,
              transition: 'background-color .2s',
            }}
      ...
      {cameraDevicesList.map(({ label, value }) => {
        ...
        return (
          <div
            ...
            className="fcr-device-dropdown__panel-item flex justify-between p-2 my-1 cursor-pointer">
            <span>{label}</span>
            {currentCameraDeviceId === value && (
              <SvgImg type={SvgIconEnum.CHECK} colors={{ iconPrimary: '#fff' }} />
            )}
          </div>
        );
      })}
...
export const ScreenShareTool = observer(() => {
  ...
  return (
    <IconButton
      iconColor="#fff"
      backgroundColor={'#343434E5'}
      icon={SvgIconEnum.SCREENSHARING}
      onClick={handleClick}
    />
  );
});
...
export const RosterTool = observer(() => {
  ...
  return (
    <div className="relative overflow-hidden">
      <div
        className="text-center w-full bottom-0 absolute"
        style={{
