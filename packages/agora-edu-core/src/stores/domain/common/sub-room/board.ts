...
import { AgoraEduClassroomEvent, EduClassroomConfig, WhiteboardTool } from '../../../..';
import { SubRoomStore } from '.';
import { WhiteboardShapeTool } from '../board/type';
import {
  rgbToHexColor,
  getJoinRoomParams,
...
import { WhiteboardConfigs } from '../../../../configs';
...
export class BoardStoreEach {
  ...
  private whiteboardConfig?: WhiteboardConfigs;
  ...
  async joinBoard(role: EduRoleTypeEnum) {
    try {
      ...
      let { boardAppId, boardId, boardToken, boardRegion } = this
        .whiteboardConfig as WhiteboardConfigs;
  ...
  private _whiteboardEventListeners: Partial<RoomCallbacks> = {
    onPhaseChanged: (phase: RoomPhase) => {
      this.subRoomStore.connectionStore.setWhiteboardState(phase);
    },
