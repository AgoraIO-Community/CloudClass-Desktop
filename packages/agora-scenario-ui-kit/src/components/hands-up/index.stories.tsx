...
export const Docs = ({ handsUpState }: any) => {
  ...
  const [list, updateList] = useState<StudentInfo[]>(
    [...'.'.repeat(10)].map((_, idx: number) => ({
      userName: `${idx}_name`,
      userUuid: `${idx}`,
      coVideo: false,
    })),
  ...
  return (
    <div
      className="flex justify-center m-screen h-screen items-end"
...
export const StudentHandUp = () => {
  ...
  return (
    <div className="flex justify-center items-center m-screen h-screen">
      <HandsUpSender state={state} onClick={() => setActive('actived')} />
    </div>
  );
};
