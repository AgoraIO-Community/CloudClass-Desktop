...
export class ReportService extends ApiBase {
  ...
  initReportUserParams(params: { sid: string; appId: string; uid: string }) {
    this.sid = params.sid;
  ...
  initReportRoomParams(params: { rid: string }) {
    this.rid = params.rid;
  }
  ...
  guardParams() {
    ...
    throw GenericErrorWrapper(
      new Error(`missing params: appId: ${this.appId}, uid: ${this.rtmUid}`),
    );
  }
