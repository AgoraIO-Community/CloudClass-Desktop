...
export class AgoraEduApi extends ApiBase {
  ...
  constructor(params: ApiBaseInitializerParams) {
    ...
    (this.authorization = ''), (this.sdkDomain = params.sdkDomain);
  ...
  async fetchUsersStreams(
    nextId: string,
    count: number,
    updateTimeOffset: number = 0,
    includeOffline: number = 0,
  ): Promise<UserStreamResponseData> {
    ...
    do {
      ...
      const streamList = data.list.reduce((acc, item) => {
        ...
        if (!combineLatestStreamIndexList.length) {
          EduLogger.warn(
            'combineLatestStreamIndexList is empty',
            newStreams,
  ...
  async subRoomData(params: any) {
    const { step, roomUuid, requestId, nextId, nextTs } = params;
    try {
      let resp = await this.fetch({
  ...
  async deleteBizStream(args: DeleteBizStreamParams): Promise<any> {
    const { roomUuid, userUuid, streamUuid } = args;
  ...
  async sendUserChatMessage(args: PeerChatMessageParams): Promise<any> {
    const { message, remoteUser, roomUuid } = args;
    let res = await this.fetch({
  ...
  async updateRoomBatchProperties({
    roomUuid,
    properties,
    cause,
  }: {
   ...
   */
  async batchUpdateRoomAttributes(
    roomUuid: string,
    properties: BatchAttributes,
    cause?: CauseType,
  ) {
    let res = await this.fetch({
      url: `/v1/rooms/${roomUuid}/properties`,
