...
export class LectureRoomStreamUIStore extends StreamUIStore {
  ...
  @action.bound
  carouselNext() {
    if (super.studentCameraStreams.length > this._carouselShowCount + this.carouselPosition) {
  ...
  get scrollable() {
    return super.studentCameraStreams.length > this._carouselShowCount;
  }
  ...
  onInstall(): void {
    ...
    this._disposers.push(
      reaction(
        () => this.studentCameraStreams,
        () => {
          if (
            this.studentCameraStreams.length - this.carouselPosition < this._carouselShowCount &&
